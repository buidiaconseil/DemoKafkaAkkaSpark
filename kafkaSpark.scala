...
val df = kafka.withWatermark("timestamp", "500 milliseconds").select($"timestamp",explode(split($"value".cast("string"), "\\s+")).as("word")).groupBy(window($"timestamp", "10 seconds"),$"word").count
